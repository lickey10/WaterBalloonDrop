...
public class Spawner : MonoBehaviour
{
    ...
    void Start()
    {
		...
		if(unitList != null && unitList.Length > 0 && unitList[(int)unitLevel] != null)
        ...
        while (spawn)
        {
            switch (spawnType)
            {
                ...
                case SpawnModes.TimeSplitWave:
                    ...
                    break;
				case SpawnModes.RandomWave:
					if (numberOfUnits < totalUnits)
					{
						SpawnUnit();

						//randomize the timeBetweenSpawns
						yield return new WaitForSeconds(Random.Range(0.5f,timeBetweenSpawns));
					}
					break;
                default:
                    spawn = false;
